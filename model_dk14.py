...
import numpy as np
from scipy.integrate import quad, simpson, cumulative_trapezoid, fixed_quad
...
class splash(constants):
    ...
    def __init__(self, Log_Rho_s, Log_Alpha, Log_R_s, Log_Rho_0, S_e, Log_R_t, Log_Beta, Log_Gamma, F_cen=1.0, R_off=0.0, R_out=1.5, R_max=40, splrmin = 0.001, splrbin=500):
    ...
    def aux_xi_2d(self,R):
        log_R = np.log10(R)
        if not self.init_spl_cen:
            dirt = self.log_xi_2d_cen(log_R)
        if log_R<self.log_xi_2d_dict['logr_min']:
            val = 10**self.log_xi_2d_dict['log_ximax']
        elif log_R>=self.log_xi_2d_dict['logr_min'] and log_R<=self.log_xi_2d_dict['logr_max'] :
            val = 10**self.spl_log_xi_2d_cen(log_R)
        else:
            val = quad(lambda x: 10**(self.log_xi_3d(np.log10(np.sqrt(R**2+x**2)))), 0, self.R_max)[0]
        return val

    def theta_avg_xi_2d(self,R,r):
        val = quad(lambda x: self.aux_xi_2d(np.sqrt(R**2 + r**2 + 2*R*r*np.cos(x))), 0 , 2*np.pi)[0]
        #val = quad(lambda x: self.aux_xi_2d(np.sqrt(R**2 + r**2 + 2*R*r*np.cos(x))), 0 , 2*np.pi, limit=60)[0]
        val = val/(2*np.pi)
        return val
    ...
    def log_xi_2d_off(self,log_R):
        ...
        lmin = 0
        lmax = 10*self.roff
        rarr = np.linspace(lmin,lmax,100)
        yarr = 0.0*rarr

        for ii,rr in enumerate(rrad):
            for jj in range(len(rarr)):
                yarr[jj] =  self.p_off(rarr[jj]) *self.theta_avg_xi_2d(rr, rarr[jj])
            aux_spl = interp1d(rarr, yarr, kind='cubic')
            val[ii] = quad(lambda x:  aux_spl(x) , lmin, lmax)[0]
        return np.log10(val)
    def log_xi_2d_cen(self,log_R):
        if self.fcen == 1.0:
            ...
            #rrad = 10**log_R
            #rrad = np.linspace(self.spl_rmin, 10**log_R.max() + 10*self.roff, self.spl_rbin)
            rrad = np.logspace(np.log10(self.spl_rmin), np.log10(10**log_R.max() + 10*self.roff), self.spl_rbin)
            _val = 0.0*rrad
        for ii,rr in enumerate(rrad):
            xx = np.linspace(0, self.R_max,1000)

            _val[ii] = np.sum(10**(self.log_xi_3d(np.log10(np.sqrt(rr**2+xx**2)))))*(xx[1]-xx[0])
            #_val[ii] = quad(lambda x: 10**(self.log_xi_3d(np.log10(np.sqrt(rr**2+x**2)))), 0, self.R_max)[0]
        val = _val*1.0/self.R_max
        #if self.fcen !=1:
        #    _spl_log_xi_2d_cen = UnivariateSpline(np.log10(rrad), np.log10(val), s=0.0, k=4)
        #    rrad = np.logspace(np.log10(self.spl_rmin), np.log10(np.max(rrad) + 10*self.roff), self.spl_rbin)
        #    val = 10**_spl_log_xi_2d_cen(np.log10(rrad))
    ...
    def log_xi_2d(self,log_R):
        if self.fcen==1.0:
            ...
            self.spl_log_xi_2d      = UnivariateSpline(log_R, np.log10(val), s=0.0, k=4)
...
#log_rho_s, log_alpha, log_r_s, log_rho_0, s_e, log_r_t, log_beta, log_gamma
    ...
    ss = splash(Log_Rho_s = log_rho_s, Log_Alpha = log_alpha, Log_R_s = log_r_s, Log_Rho_0 = log_rho_0, S_e = s_e, Log_R_t = log_r_t, Log_Beta = log_beta, Log_Gamma = log_gamma, F_cen = f_cen, R_off = r_off, splrmin=0.01, splrbin=100)
    ...
    rad = np.logspace(np.log10(0.1),np.log10(5), 9)
