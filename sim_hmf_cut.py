...
for ii,omgm in enumerate(omgmarr):
    for jj,sigma8 in enumerate(sigma8arr):
        r200mmat[ii,jj], gammamat[ii,jj], gamma_zhao09mat[ii, jj] = get_r200m(omgm, sigma8)
        rspmat[ii,jj] = get_rsp(r200mmat[ii,jj], omgm, gammamat[ii,jj])
        rsp_zhao09mat[ii,jj] = get_rsp(r200mmat[ii,jj], omgm, gamma_zhao09mat[ii,jj])
...
im = plt.imshow(rsp_zhao09mat.T, origin='lower', aspect='auto',
                extent=[omgmarr[0], omgmarr[-1], sigma8arr[0], sigma8arr[-1]],vmin=1.1,vmax=1.5)
plt.ylabel(r'$\sigma_8$', fontsize=12)
plt.xlabel(r'$\Omega_{\rm m}$', fontsize=12)
