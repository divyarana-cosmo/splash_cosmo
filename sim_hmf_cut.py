# there are inconsistencies in input cosmologidal parameters
# needs clean up
...
class splashsim():
    ...
    def gamma_zhao09(self, omgm, sigma8, logmh):
        ...
        if 1:
            ...
            ziz,Miz,c,rhohiz,R,V,Ms,rhos,Rs,Vs,Miz_200c,c_200c,rhohiz_200c,Miz_200m,c_200m,rhohiz_200m,uniage_iz = np.loadtxt(flist[0], skiprows=1, unpack=1)
            #data = np.loadtxt(flist[0], skiprows=1)
            z  = ziz
            Mz = Miz_200m
            gamma  = (np.log10(Mz[3]) - np.log10(Mz[0]))/(np.log10(1+z[0]) - np.log10(1+z[3]))
            print('look here')
            os.system('rm mchistory*')
            os.system('mkdir -p ./outputs/')
            np.savetxt('outputs/res_%s'%fname,np.transpose([logmh, gamma]) )
        logmh, gamma = np.loadtxt('outputs/res_%s'%fname)
        return gamma
    ...
    def get_r200m(self, omgm, sigma8):
        # Setup the cosmology
        params = {'flat': True, 'H0': 100*self.h, 'Om0': omgm, 'Ob0': self.Ob0, 'sigma8': sigma8, 'ns': self.nspec, 'Tcmb0': self.tcmb}
        ...
        gammazhao09 = self.gamma_zhao09(omgm, sigma8, np.log10(avg_mh))
        rsp     = self.get_rsp(r200m, omgm, gammazhao09)

        return  r200m, gamma, gammazhao09
...
if __name__ == "__main__":
    ...
    rsp_zhao09mat   = np.zeros((len(omgmarr), len(sigma8arr)))
    ...
    for ii,omgm in enumerate(omgmarr):
        for jj,sigma8 in enumerate(sigma8arr):
            r200mmat[ii,jj], gammamat[ii,jj], gamma_zhao09mat[ii, jj] = ss.get_r200m(omgm, sigma8)

            rspmat[ii,jj] = ss.get_rsp(r200mmat[ii,jj], omgm, gammamat[ii,jj])

            rsp_zhao09mat[ii,jj] = ss.get_rsp(r200mmat[ii,jj], omgm, gamma_zhao09mat[ii,jj])
    ...
    plt.subplot(3,3,3)
    im = plt.imshow(gamma_zhao09mat.T, origin='lower', aspect='auto',
                    extent=[omgmarr[0], omgmarr[-1], sigma8arr[0], sigma8arr[-1]])
    plt.ylabel(r'$\sigma_8$', fontsize=12)
    plt.xlabel(r'$\Omega_{\rm m}$', fontsize=12)
    ...
    im = plt.imshow(rspmat.T, origin='lower', aspect='auto',
                    extent=[omgmarr[0], omgmarr[-1], sigma8arr[0], sigma8arr[-1]])
                    #extent=[omgmarr[0], omgmarr[-1], sigma8arr[0], sigma8arr[-1]],vmin=1.4, vmax=2.0)
    plt.ylabel(r'$\sigma_8$', fontsize=12)
    plt.xlabel(r'$\Omega_{\rm m}$', fontsize=12)
    ...
    plt.subplot(3,3,5)
    im = plt.imshow(rsp_zhao09mat.T, origin='lower', aspect='auto',
                    extent=[omgmarr[0], omgmarr[-1], sigma8arr[0], sigma8arr[-1]])
                    #extent=[omgmarr[0], omgmarr[-1], sigma8arr[0], sigma8arr[-1]],vmin=1.4,vmax=2.0)
    plt.ylabel(r'$\sigma_8$', fontsize=12)
    plt.xlabel(r'$\Omega_{\rm m}$', fontsize=12)
    plt.colorbar(im, label=r'$r_{\rm sp}(\Gamma_{\rm zhao+09})$')
    ...
    plt.ylabel(r'$\sigma_8$', fontsize=12)
    plt.xlabel(r'$\Omega_{\rm m}$', fontsize=12)
    plt.colorbar(im, label=r'$r_{\rm sp}(\Gamma_{\rm zhao+09})/r_{\rm 200m}$')
    plt.tight_layout()
