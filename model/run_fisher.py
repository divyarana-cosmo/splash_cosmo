...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n--- Fisher Forecast Constraints (1-sigma) ---")
    ...
    # ---------------------------------------------------------
    # F. GetDist Visualization
    # ---------------------------------------------------------
    print("\nRendering GetDist Triangle Plot...")
    from getdist import plots
    from getdist.gaussian_mixtures import GaussianND
    
    fisher_base = GaussianND(xtrue, cov_data, names=param_names, labels=param_labels, label='Data Only')
    fisher_shape = GaussianND(xtrue, cov_shape, names=param_names, labels=param_labels, label='+ DK14 Priors')
    fisher_all = GaussianND(xtrue, cov_tot, names=param_names, labels=param_labels, label='+ Planck')
